...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    /*
    INKAction *storeFlickr = [INKAction action:@"Store in Flickr" type:INKActionType_Send];
    [Ink registerAction:storeFlickr withTarget:self selector:@selector(storeToFlickr:)];
    
    INKAction *storePicasa = [INKAction action:@"Store in Picasa" type:INKActionType_Publish];
    [Ink registerAction:storePicasa withTarget:self selector:@selector(storeToPicasa:)];

    INKAction *storeBox = [INKAction action:@"Store in Box" type:INKActionType_Annotate];
    [Ink registerAction:storeBox withTarget:self selector:@selector(storeToBox:)];
     
    INKAction *storeSkyDrive = [INKAction action:@"Store in SkyDrive" type:INKActionType_Attach];
    [Ink registerAction:storeSkyDrive withTarget:self selector:@selector(storeToSkyDrive:)];
     */
...
- (void)storeBlob:(INKBlob *)blob withStartingSource:(NSString *)startingSource {
    ...
    if ([[self.window.rootViewController presentedViewController] class] == [FPSaveController class] ){
